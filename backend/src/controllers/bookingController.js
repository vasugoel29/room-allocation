...
export const createBooking = async (req, res) => {
  try {
    const result = await bookingService.createBookingHandler(req.body, req.user);
    if (result.error) {
...
export const cancelBooking = async (req, res) => {
  try {
    const result = await bookingService.cancelBookingHandler(req.params.id, req.user);
    if (result.error) {
      return res.status(result.status).json({ error: result.error });
    }
...
export const rescheduleBooking = async (req, res) => {
  try {
    const result = await bookingService.rescheduleBookingHandler(req.params.id, req.body, req.user);
    if (result.error) {
      return res.status(result.status).json({ error: result.error });
    }
