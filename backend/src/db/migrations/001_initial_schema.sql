...
CREATE OR REPLACE TRIGGER trg_users_role_scope BEFORE INSERT OR UPDATE ON users
...
CREATE INDEX IF NOT EXISTS idx_rooms_search ON rooms (capacity, has_ac, has_projector)
...
CREATE INDEX IF NOT EXISTS idx_timetable_faculty ON timetable_slots(faculty_id, day_of_week, start_time);
CREATE INDEX IF NOT EXISTS idx_timetable_room ON timetable_slots(room_id, day_of_week, start_time);
CREATE INDEX IF NOT EXISTS idx_timetable_branch_sem ON timetable_slots(branch_id, semester, section);

CREATE TABLE IF NOT EXISTS timetable_slot_overrides (
  id                SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_slot_overrides_date ON timetable_slot_overrides(override_date);
...
CREATE INDEX IF NOT EXISTS idx_bookings_room_range_gist ON bookings USING GIST (room_id, tstzrange(start_time, end_time));
CREATE INDEX IF NOT EXISTS idx_bookings_user_range_gist ON bookings USING GIST (created_by, tstzrange(start_time, end_time));

CREATE OR REPLACE TRIGGER trg_bookings_updated_at BEFORE UPDATE ON bookings
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
CREATE INDEX IF NOT EXISTS idx_requests_requester ON requests(requested_by, status);
CREATE INDEX IF NOT EXISTS idx_requests_reviewer ON requests(reviewed_by, status);

CREATE OR REPLACE TRIGGER trg_requests_updated_at BEFORE UPDATE ON requests
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
CREATE OR REPLACE TRIGGER trg_requests_reviewed_at BEFORE UPDATE ON requests
  FOR EACH ROW EXECUTE FUNCTION set_reviewed_at();

CREATE UNIQUE INDEX IF NOT EXISTS idx_one_pending_promotion
...
CREATE INDEX IF NOT EXISTS idx_booking_requests_room_time
...
CREATE INDEX IF NOT EXISTS idx_transfer_requests_owner ON transfer_requests(owner_id);
CREATE INDEX IF NOT EXISTS idx_transfer_requests_target ON transfer_requests(target_faculty_id);
...
CREATE OR REPLACE TRIGGER trg_one_pending_transfer BEFORE INSERT ON transfer_requests
...
CREATE INDEX IF NOT EXISTS idx_cancellation_faculty ON cancellation_requests(faculty_id);
...
CREATE OR REPLACE TRIGGER trg_one_pending_cancellation BEFORE INSERT ON cancellation_requests
...
-- 8. Views & Functions
-- =====================================================================
CREATE OR REPLACE VIEW faculty_schedule AS
SELECT
...
CREATE OR REPLACE VIEW room_occupancy_today AS
