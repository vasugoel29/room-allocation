              ...
              HOURS.map(hour => (
                <div key={hour} className={`flex-1 grid group border-b border-border last:border-b-0 calendar-transition ${viewMode === 'day' ? 'grid-cols-[50px_1fr] sm:grid-cols-[120px_1fr]' : 'grid-cols-[50px_repeat(5,1fr)] sm:grid-cols-[120px_repeat(5,1fr)]'}`}>
                  ...
                  {displayDays.map(dateStr => {
                    ...
                    <div 
                      key={dateStr} 
                      className={`p-1 border-l border-border hover:bg-bg-primary/30 transition-colors cursor-pointer relative h-full min-h-[70px] sm:min-h-[80px] overflow-hidden ${viewMode === 'day' ? 'flex items-center' : ''}`}
                      onClick={() => {
                        const dateObj = currentWeekDay?.fullDate;
                        onSlotClick({ day: dayLabel, hour, date: dateObj });
                      }}
                    >
                      <div className="p-1.5 h-full calendar-transition slot-scroll no-scrollbar items-center">
                          {rooms
                          .filter(room => {
                          ...
                          .map(room => { 
                            ...
                            return (
                              <div 
                                ...
                                className={`px-3 py-2 rounded-xl border shadow-sm ${!booking ? 'hover:translate-y-[-1px] active:scale-95' : ''} transform transition-all text-sm leading-tight truncate flex items-center justify-between font-black border-border min-w-[140px] sm:min-w-[200px] ${booking ? 'bg-bg-primary opacity-60 grayscale-[0.3] cursor-not-allowed' : 'bg-bg-secondary text-text-primary cursor-pointer'}`}
